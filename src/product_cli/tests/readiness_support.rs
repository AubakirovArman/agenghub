...
pub(super) struct ReadinessFixture {
    ...
    pub roadmap: std::path::PathBuf,
    pub evidence: std::path::PathBuf,
    pub history: std::path::PathBuf,
    pub kimi: std::path::PathBuf,
}

impl ReadinessFixture {
    pub(super) fn ready() -> Result<Self> {
        Self::new(false)
    }

    pub(super) fn blocked_kimi() -> Result<Self> {
        Self::new(true)
    }
...
pub(super) fn with_readiness_fixture(
    ...
    with_env_vars(
        &[
            (
                "AGENTHUB_API_AUDIT_EVIDENCE",
                Some(fixture.evidence.display().to_string()),
            ),
            (
