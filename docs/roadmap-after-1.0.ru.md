...
3. Run `agenthub readiness next --json --check` first for the single prioritized current action, then `agenthub readiness blockers --json --check` for the short machine-readable blocker list, then `agenthub readiness checklist --json --check` for the prompt-to-artifact requirement map, then `agenthub readiness evidence --json --check` for the focused dogfood/evidence counters, then `agenthub readiness audit --json --check` for the full gate. These commands refresh RC evidence without contaminating stdout and expose top-level `blocked_checks`, so CI can parse JSON even while the command exits non-zero for Kimi/RC blockers. For human review use `agenthub readiness next --check`, `agenthub readiness evidence --check`, and `agenthub readiness audit --check`; inspect `blocked_checks`, per-check `next_commands`/`check_next` rows, inspect `target/dogfood/rc-evidence.jsonl`, fill any remaining real daily evidence, and pass `scripts/rc-dogfood-gate.sh --check` without lowering thresholds. `scripts/api-native-completion-audit.sh --json --check` remains a compatible script-level evidence path with matching blocked-check IDs.
...
6. Use `agenthub providers status --json` for raw provider state, `agenthub providers recovery --json` for redacted recovery automation, `agenthub readiness next --json` for the current action plan, and `agenthub readiness audit --json` for final readiness gates without parsing human-readable status text.
