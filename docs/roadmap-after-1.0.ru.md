...
These are the next concrete engineering steps from the current `0.4.67-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.67` only adds a visible, disabled-by-default ecosystem planning surface.
...
| `1.0 RC` | Real dogfooding gate | Collect and pass the `0.4.67` evidence gate with real daily usage, including stable resume/rewind/stats, 20+ Ops and 20+ project-edit flows, passed DeepSeek/Kimi provider dogfood, and no Kimi/auth/latency/approval blockers |
...
6. Use `agenthub ecosystem status --json` as the machine-readable MCP/A2A planning surface while 1.0 remains blocked.
7. Post-1.0: start MCP stdio client runtime only after Chat/Ops/Project, memory review, TUI visibility, and Ops host safety are stable in daily use.
