...
These are the next concrete engineering steps from the current `0.4.56-local-preview` bridge toward `1.0`. They are intentionally before MCP/A2A and marketplace work.
...
| `1.0 RC` | Real dogfooding gate | Collect and pass the `0.4.56` evidence gate with real daily usage, including stable resume/rewind/stats, 20+ Ops and 20+ project-edit flows, passed DeepSeek/Kimi provider dogfood, and no Kimi/auth/latency/approval blockers |
...
2. After receiving a valid Kimi/Moonshot key, run `providers rc-unblock kimi --from-file <new-key-file>`; it installs the key without secret output, then performs `providers test kimi`, `kimi-auth-check`, live Kimi provider dogfood, RC evidence collection, and the RC gate in the correct order.
