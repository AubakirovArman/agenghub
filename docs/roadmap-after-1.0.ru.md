...
These are the next concrete engineering steps from the current `0.4.25-local-preview` bridge toward `1.0`. They are intentionally before MCP/A2A and marketplace work.
...
| `0.4.26` | TUI live tool cards | Show provider tool calls, tool-result reinjection, approval-required stops, memory extraction, cost/tokens, and dashboard links directly in the terminal transcript/event rail |
...
This is why the `v0.4.8` through `v0.4.25` bridge releases focus on global Chat/Ops memory, a review-gated memory inbox, budgeted memory-aware chat context, provider diagnostics, visible mode routing, explainable tool permissions, lazy project bootstrap, context compaction receipts, event-backed TUI visibility, visible transaction approval receipts, CI-friendly headless approval receipts, recoverable session reads, native DeepSeek/Kimi command-plan tool-call receipts, dashboard observability, API-native tool-result reinjection, tool registry policy hardening, and review-only automatic memory extraction rather than starting MCP/A2A early.

## Next Implementation Sequence

1. `0.4.26`: surface live provider tool calls, tool-result receipts, memory extraction, and approval stops in the terminal TUI, not only dashboard/report artifacts.
2. `0.4.27`: improve memory inbox review with grouping, conflicts, confidence bands, and batch decisions.
3. `0.4.28`: make Ops Mode safer and more useful with host profiles, host-scoped memory, and reusable runbook receipts.
