...
3. Run `agenthub readiness blockers --json --check` first for the short machine-readable blocker list, then `agenthub readiness checklist --json --check` for the prompt-to-artifact requirement map, then `agenthub readiness evidence --json --check` for the focused dogfood/evidence counters, then `agenthub readiness audit --json --check` for the full gate. These commands refresh RC evidence without contaminating stdout and expose top-level `blocked_checks`, so CI can parse JSON even while the command exits non-zero for Kimi/RC blockers. For human review use `agenthub readiness evidence --check` plus `agenthub readiness audit --check`, inspect `blocked_checks`, per-check `next_commands`/`check_next` rows, inspect `target/dogfood/rc-evidence.jsonl`, fill any remaining real daily evidence, and pass `scripts/rc-dogfood-gate.sh --check` without lowering thresholds. `scripts/api-native-completion-audit.sh --json --check` remains a compatible script-level evidence path with matching blocked-check IDs.
