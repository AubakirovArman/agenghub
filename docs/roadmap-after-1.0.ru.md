...
| `0.4.117` | Kimi unblock and RC refresh | Run the replacement-key path when a valid Moonshot key is available, refresh live Kimi provider dogfood, collect RC evidence, and keep readiness blocked only on genuinely unresolved dogfood items |
| `0.4.118` | Live dogfood observability polish | Turn successful Kimi unblock plus RC refresh into a short operator receipt: provider, model, endpoint, token/cost receipt, dogfood run id, readiness completion status, and remaining blockers |
