...
These are the next concrete engineering steps from the current `0.4.105-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.104` added `agenthub readiness completion --json` as the aggregate final completion bundle over readiness-next/checklist/evidence/provider status, and `0.4.105` adds shell plan aliases (`/mode chat|devops|project`, `/cost`, `/balance`, `/hosts`, `/connect <host>`) so the interactive entrypoint matches the API-native UX plan without enabling any external CLI providers.
