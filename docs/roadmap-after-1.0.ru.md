...
These are the next concrete engineering steps from the current `0.4.109-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.104` added `agenthub readiness completion --json` as the aggregate final completion bundle, `0.4.105` added shell plan aliases for mode/cost/Ops host controls, `0.4.106` makes `/sessions` a first-class chat-session alias while keeping transaction history on `/transactions`, `/tx`, and `/history`, `0.4.107` splits lightweight provider selection from provider setup, `0.4.108` release-gates the daily shell UX aliases without Git/`.agent` bootstrap, and `0.4.109` makes that shell UX gate visible in the readiness checklist.
