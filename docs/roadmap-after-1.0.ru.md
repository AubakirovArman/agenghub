...
These are the next concrete engineering steps from the current `0.4.76-local-preview` bridge toward `1.0`. MCP/A2A runtime work still waits for the 1.0 gate; `0.4.67` added a visible, disabled-by-default ecosystem planning surface, `0.4.68` adds machine-readable provider status, `0.4.69` turns that status into a redacted provider recovery report for DeepSeek/Kimi automation, `0.4.70` makes the final completion audit machine-readable for release gates, `0.4.71` exposes that gate as first-class product CLI, `0.4.72` fixes Windows CI parity for that readiness surface, `0.4.73` adds a focused readiness blocker view for release stabilization, `0.4.74` expands the post-1.0 ecosystem surface to all strategic tracks from the external roadmap, `0.4.75` makes that surface part of the product readiness audit, and `0.4.76` makes open RC blocker IDs visible in the readiness blocker detail.
