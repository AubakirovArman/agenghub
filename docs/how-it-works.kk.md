...
4. The selected provider route or internal deterministic runner writes prompt/invocation artifacts, then `execution.commands` run in that isolated worktree.
...
- `agent.adapter`: backward-compatible route selector; `deepseek` and `kimi` are API-native provider routes, while `command` is the internal deterministic runner.
...
AgentSpec still uses `agent.adapter` for compatibility with existing specs. In product UX, user-facing AI providers are API-native `deepseek` and `kimi`; `command` is the internal deterministic runner used for fixtures, tests, and explicit command specs.

API-native routes use AgentHub-owned DeepSeek/Kimi requests and generated prompts:
...
Detailed route behavior is documented in [Provider routes](agent-adapters.en.md).
...
- `.agent/tx/<tx-id>/agent_transcript.jsonl`: provider route and command transcript.
- `.agent/tx/<tx-id>/agent_prompt_<role>.md`: role prompt artifact.
- `.agent/tx/<tx-id>/agent_prompt_<role>.md`: prompt artifact for API-native provider routes.
