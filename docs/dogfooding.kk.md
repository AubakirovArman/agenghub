...
After receiving a valid Kimi/Moonshot key, `providers rc-unblock kimi --from-file <new-key-file>` installs it without printing the secret and then runs the full unblock sequence in order: `agenthub providers test kimi`, `scripts/kimi-auth-check.sh`, live Kimi provider dogfood, `scripts/rc-evidence-collect.sh`, and `scripts/rc-dogfood-gate.sh --check`. The existing two-step path still works after separate rotation with `providers rotate-key kimi`. If the provider test still fails, the command runs `scripts/kimi-auth-check.sh` anyway as diagnostics so the redacted two-endpoint auth report is refreshed before it returns `blocked`.
