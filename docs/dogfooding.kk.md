...
After rotating a valid Kimi/Moonshot key, `providers rc-unblock kimi` runs the full unblock sequence in order: `agenthub providers test kimi`, `scripts/kimi-auth-check.sh`, live Kimi provider dogfood, `scripts/rc-evidence-collect.sh`, and `scripts/rc-dogfood-gate.sh --check`. If the provider test still fails, the command runs `scripts/kimi-auth-check.sh` anyway as diagnostics so the redacted two-endpoint auth report is refreshed before it returns `blocked`.
