...
4. Selected provider route или internal deterministic runner writes prompt/invocation artifacts, then `execution.commands` run in that isolated worktree.
...
- `agent.adapter`: backward-compatible route selector; `deepseek` и `kimi` are API-native provider routes, while `command` is the internal deterministic runner.
