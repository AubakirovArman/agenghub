...
The working backlog is in `prd/v2/`. Effect ledger, rollback handlers, resume/retry/resolve, Smart Sync, Workspace Runtime trait, LLM Provider Gateway, VCM-OS typed memory, Hardened Runner, and AAL v2 semantics are done. The next task is Adaptive Orchestration.
