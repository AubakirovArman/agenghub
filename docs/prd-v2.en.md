...
The working backlog is in `prd/v2/`. Effect ledger, rollback handlers, resume/retry/resolve, Smart Sync, Workspace Runtime trait, LLM Provider Gateway, VCM-OS typed memory, Hardened Runner, AAL v2 semantics, and Adaptive Orchestration are done. The next task is Verifier Integrations v2.
