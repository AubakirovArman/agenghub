...
The working backlog is in `prd/v2/`. Effect ledger, rollback handlers, resume/retry/resolve, Smart Sync, Workspace Runtime trait, LLM Provider Gateway, VCM-OS typed memory, Hardened Runner, AAL v2 semantics, Adaptive Orchestration, Verifier Integrations v2, Governance v2, and Analytics History are done. The next task is Specialized Domain Runtimes.
