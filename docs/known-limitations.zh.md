...
AgentHub `0.4.9-local-preview` is an installable local developer preview, not a stable public or enterprise product.
...
`deepseek` and `kimi` support OpenAI-compatible HTTP and HTTPS endpoints with bearer-token auth, timeouts, structured error bodies, streaming chat, and API-native project command execution. The broader structured tool loop, full-screen TUI, and automatic memory extraction are still in progress.
