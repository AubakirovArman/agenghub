...
grep -q 'credential_warning: Kimi Code CLI OAuth credentials are not Moonshot OpenAI-compatible API keys; create a plain Moonshot API key instead' "$TMP/out.txt"
grep -q '"status": "blocked"' "$REPORT"
grep -q '"auth_key_sha256_12": "abc123abc123"' "$REPORT"
grep -q '"credential_warning": "Kimi Code CLI OAuth credentials are not Moonshot OpenAI-compatible API keys; create a plain Moonshot API key instead"' "$REPORT"
