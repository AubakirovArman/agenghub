...
- 事务型执行内核；
- 基于 git worktree 隔离的 `CodeWorkspace`、`ContentWorkspace`、`DataWorkspace`、`InfraWorkspace`、`MediaWorkspace`；
